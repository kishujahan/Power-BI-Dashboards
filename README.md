# Power BI Portfolio: The Evolution of a Data Jobs Dashboard

Building upon the foundations of my initial Data Jobs analysis, Data Jobs Dashboard 2.0 represents a significant technical leap in my Power BI journey. While the first version focused on core visualization and drill-through reporting, this iteration emphasizes dynamic user interaction and advanced data modeling.

This dashboard follows the architectural framework designed by [Luke Barousse](https://youtu.be/FwjaHCVNBWA?si=UvGyjyEYt34hRCKb). While the core metrics and parameters align with the tutorial, the development process involved significant independent troubleshooting, particularly in interface optimization, layer management, and precision formatting to ensure a production-ready finish.

# 📊 Featured Dashboards

Each dashboard includes a detailed README documenting the details and features.

## Data Jobs Dashboard V1

### Overview Page :

![](/images/proj1_page1.PNG)

### Drill-Through Page :

![](/images/proj1_page2.PNG)

### Key Skills Used:

- Dashboard Layout & Design
- Data Transformation using Power Query ETL
- Data Modeling with Table Relationships
- Standard Charts, Implicit Measures & Aggregation
- Geographic Data Visualization using Map Charts
- KPI Tracking & Multi-Dimensional Matrices
- Dynamic Data Filtering via Slicers
- Bookmark-Driven Navigation Logic
- Advanced Drill-Through Actions

[**View Project 1: Foundational Dashboard Documentation ➔**](/Data_jobs_v1/README.md)


## Data Jobs Dashboard V2

![](/images/Dashboard_2.0.PNG)

### Key Skills Used:

- Enhanced Dashboard Design
- Complex Power Query Transformations
- Standard charts & Aggregation
- Explicit DAX Measures 
- Interactive Visualizations (Field Parameters & Slicers) 
- Dynamic KPI Integration 
- Advanced Filter Logic (Optimized Slicers & Cross-Filtering)

[**View Project 2: Advanced Dashboard Documentation ➔**](/Data_jobs_v2/README.md)
